...
GeoLayoutCommandProc GeoLayoutJumpTable[] = {
    ...
    /*GEO_CMD_JUMP_TO_EXISTING_GRAPH_NODE */ geo_layout_cmd_jump_to_existing_graph_node,
};
...
void geo_layout_cmd_jump_to_existing_graph_node(void) {
    ...
    graphNode = init_graph_node_pointer(gGraphNodePool, NULL, (u16) cur_geo_cmd_s16(0x02));
    register_scene_graph_node(&graphNode->node);
    gGeoLayoutCommand += 0x04 << CMD_SIZE_SHIFT;
}
