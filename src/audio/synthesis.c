...
#ifdef UCODE_LOW_PASS_FILTER
// NOTE: This does not function as a high-pass filter, and only supports intensities up to 0x8000
...
#define LPF_SCALE 16384
    ...
    tmpCoefs[tmp++] = diff;
    while (tmp < ARRAY_COUNT(tmpCoefs)) {
        coef *= mult;
        tmpCoefs[tmp++] = coef;
    }
...
#ifdef UCODE_LOW_PASS_FILTER
    note_init_lpf(&note->lpf, 0, 1.0f, TRUE, note->synthesisBuffers->lpfState, note->synthesisBuffers->lpfCoefs); // This may be run at any given time
#endif
}
