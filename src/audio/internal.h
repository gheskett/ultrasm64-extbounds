...
#endif
#ifdef UCODE_LOW_PASS_FILTER
    // In practice, lpfState only needs to be 8 bytes, but due to caching issues, this ends up being padded to a multiple of 16 bytes.
    // This data cannot safely be stored within the actual AudioLPFilter struct (again due to cache), so it goes here instead.
    s16 lpfState[0x8];
    s16 lpfCoefs[0x10];
#endif
#endif
